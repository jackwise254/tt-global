...
<br/>
  ...
  <h4 class="text-center mb-1"> <u>Delivery Note</u></h4>
    <div class="col-6">
        ...
        <!-- <form>
            <div id="custom-search-input">
                <div class="input-group">
                    <input type="text" class="col-6 rounded-pill " id="search" name="search" placeholder='scan here' autofocus required>
        ...
        <div class="col-md-6">
        <form name="test" class="col-7 " action="" method="POST">
        ...
        <input type="text" class="col-6 rounded-pill " id="search" name="search" placeholder='scan here' autofocus required>
            ...
            <button type ="submit" class="d-none" id="myBtn" onchange="this.form.submit()"></button>
            <button type="button" onchange="refreshPage()" class='d-none'>Close</button>
 ...
 $(document).ready(function() {
    $( "#search" ).autocomplete({
        source: function(request, response) {
            $.ajax({
            url: BASE_URL + "/ProductsCrud/delvsub",
            data: {
                    term : request.term
             },
            dataType: "json",
    ...
    $(document).ready(function() {
        ...
        $("#pid").autocomplete({
            ...
            source: function(request, response) {
                ...
                // Fetch data
                $.ajax({
                    url: "<?php echo base_url('ProductsCrud/delvs');?>",
                    type: 'post',
                    dataType: "json",
                    data: {
                        search: request.term
            ...
            select: function(event, ui) {
                // Set selection
                $('#pid').val(ui.item.pid); // display the selected text
                return false;
            },
